...
const MenuItem = ({ icon, label, sublabel, onPress, danger, trailing }) => (
  <TouchableOpacity
    ...
    <View style={menuStyles.textBlock}>
      ...
      {sublabel ? <Text style={menuStyles.sublabel}>{sublabel}</Text> : null}
    </View>
    {trailing ? trailing : (onPress ? <Icon name={ICONS.forward} size={22} color={Colors.textMuted} /> : null)}
  </TouchableOpacity>
);
...
const ProfileScreen = ({ navigation }) => {
  ...
  const { products } = useProducts({ allShops: true });
  ...
  const handleToggleNotifications = async (value) => {
    ...
    if (user?.uid) {
      await updateNotificationSettings(user.uid, value);
    }
    if (!value) {
      try {
        cancelAllAlerts();
      } catch (error) {
        console.warn('[ProfileScreen] Failed to cancel alerts:', error);
      }
    }
  };
        ...
        <View style={styles.profileCard}>
          ...
          <View style={styles.profileMetaRow}>
            ...
            <View style={styles.storeBadge}>
              <Icon name={ICONS.shop} size={14} color={Colors.white} />
              <Text style={styles.storeBadgeText}>{shops.length} Stores</Text>
            </View>
          </View>
          <Text style={styles.activeStoreText}>
            {activeShop ? `Active store: ${activeShop.name}` : 'No active store selected'}
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.menuCard}>
            ...
            <MenuItem
              icon={ICONS.shop}
              label="My Stores"
              sublabel="Add stores and switch active inventory"
        ...
        <SafeAreaView style={modalStyles.container}>
          ...
          <ScrollView style={modalStyles.body}>
            <Text style={modalStyles.text}>
              Your privacy is important to us. ShelfSafe stores account, store, and product information in Firestore so we can keep separate inventories per store and send expiry reminders when you allow notifications.
            </Text>
          </ScrollView>
        </SafeAreaView>
      </Modal>
      ...
      <Modal
        ...
        <SafeAreaView style={modalStyles.container}>
          ...
          <View style={modalStyles.bodyCenter}>
            ...
            <Text style={modalStyles.appDesc}>
              The smart way to manage expiry inventory across multiple stores.
            </Text>
          </View>
        </SafeAreaView>
      </Modal>
